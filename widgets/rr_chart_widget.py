...
Premium respiratory-rate reference chart widget for the
CST Health Monitoring Station kiosk.

Why this file matters:
- It is the reusable respiratory-rate detail visualization widget used by:
    - screens/rr_detail_screen.py
    - screens/results_screen.py
    - screens/diagnosis_screen.py
- It is designed to work with the uploaded respiratory-rate artwork:
    - assets/detail_graphics/rr_reference_chart.png
- It supports both:
    - image-backed mode, where the premium chart asset is shown and PyQt
      overlays the current respiratory-rate marker / active state / recent trend
...
@dataclass
class RRRangeDefinition:
    """
    Normalized respiratory-rate range definition.
...
DEFAULT_RR_RANGES: List[RRRangeDefinition] = [
    RRRangeDefinition(
        label="Low",
        minimum=None,
        maximum=12.0,
        severity=SEVERITY_ATTENTION,
        description="Respiratory rate is below the usual resting adult reference range.",
        color_hex="#FFD25E",
        short_label="Low",
    ),
    RRRangeDefinition(
        ...
        description="Respiratory rate is within the common adult resting reference range.",
        color_hex="#3FE28F",
        short_label="Normal",
    ),
    RRRangeDefinition(
        ...
        description="Respiratory rate is above the typical resting range and should be monitored.",
        color_hex="#FFA14D",
    ...
    RRRangeDefinition(
        ...
        description="Respiratory rate is significantly elevated and may need urgent review.",
        color_hex="#FF6E88",
        short_label="High",
    ),
]
...
class RRChartTheme:
    """
    Theme container for RRChartWidget and canvas rendering.
    """
...
DEFAULT_RR_CHART_THEME = RRChartTheme()
...
class _RRChartCanvas(QWidget):
    """
    Internal canvas that paints:
    - optional artwork asset
    - fallback respiratory-rate reference chart
    - animated current respiratory-rate marker
    - active range highlighting
    - optional recent respiratory-rate trend
    """
    ...
    def __init__(
        ...
        *,
        compact: bool = False,
        theme: RRChartTheme = DEFAULT_RR_CHART_THEME,
    ) -> None:
        super().__init__(parent)

        self._compact = bool(compact)
        self._ultra_compact = bool(self._compact and (KIOSK_WIDTH <= 800 or KIOSK_HEIGHT <= 480))
        self._theme = theme

        self._asset_path = ""
        self._asset_pixmap = QPixmap()

        self._ranges: List[RRRangeDefinition] = list(DEFAULT_RR_RANGES)
        self._scale_min = 6.0
        self._scale_max = 30.0

        self._target_value: Optional[float] = None
        self._display_value: float = 0.0
        self._active_index: int = -1
        self._recent_values: List[float] = []
        self._unit = "breaths/min"
        ...
        self.setMinimumHeight(230 if not self._compact else (168 if self._ultra_compact else 190))
    ...
    def _current_range(self) -> Optional[RRRangeDefinition]:
    ...
    def _sync_compact_mode(self, force: bool = False) -> None:
        width = self.width() if self.width() > 0 else int(KIOSK_WIDTH)
        height = self.height() if self.height() > 0 else int(KIOSK_HEIGHT)

        compact_now = bool(self._compact or IS_COMPACT_KIOSK or width <= 820 or height <= 500)
        ultra_now = bool((KIOSK_WIDTH <= 800 and KIOSK_HEIGHT <= 480) or width <= 760 or height <= 430)

        if not force and compact_now == self._compact and ultra_now == getattr(self, "_ultra_compact", False):
            return
    ...
    def set_ranges(self, ranges: Iterable[RRRangeDefinition]) -> None:
        normalized = [item for item in ranges if isinstance(item, RRRangeDefinition)]
        if normalized:
    ...
    def set_unit(self, unit: str) -> None:
        self._unit = safe_str(unit, "breaths/min").strip() or "breaths/min"
        self.update()
    ...
    def _draw_current_marker(self, painter: QPainter, plot_rect: QRectF, latest_point: Optional[QPoint]) -> None:
        ...
        value_text = f"{self._format_value(self._display_value)}"
        pill_w = 92.0 if not self._compact else 78.0
        pill_rect = QRectF(
        ...
        painter.drawText(pill_rect, int(Qt.AlignmentFlag.AlignCenter), f"{value_text} {self._unit}")
...
class RRChartWidget(QFrame):
    """
    Premium respiratory-rate chart widget.

    Main capabilities:
    - polished frame container
    - title / subtitle
    - animated respiratory-rate reference chart
    ...
    def __init__(
        ...
        parent: Optional[QWidget] = None,
        *,
        title: str = "Respiratory Rate Chart",
        subtitle: str = "Resting respiratory-rate range visualization",
        ...
        theme: Optional[RRChartTheme] = None,
        minimum_height: int = 360,
    ) -> None:
        super().__init__(parent)

        self._logger = logger.bind(component="RRChartWidget")
        self._theme = theme or DEFAULT_RR_CHART_THEME
        self._compact = bool(compact) or bool(IS_COMPACT_KIOSK)
        self._ultra_compact = bool((KIOSK_WIDTH <= 800 or KIOSK_HEIGHT <= 480) and self._compact)
        ...
        self.setObjectName("RRChartWidget")
        ...
        self.setMinimumHeight(max(220, int(minimum_height if not self._compact else (minimum_height - 84 if self._ultra_compact else minimum_height - 48))))
        ...
        self.set_ranges(DEFAULT_RR_RANGES)
    ...
    def _build_ui(self) -> None:
        ...
        root.setContentsMargins(
            12 if not self._compact else (8 if self._ultra_compact else 10),
            10 if not self._compact else (7 if self._ultra_compact else 8),
            12 if not self._compact else (8 if self._ultra_compact else 10),
            10 if not self._compact else (7 if self._ultra_compact else 8),
        )
        root.setSpacing(8 if not self._compact else (4 if self._ultra_compact else 6))
        ...
        self._canvas = _RRChartCanvas(self, compact=self._compact, theme=self._theme)
        self._canvas._ultra_compact = self._ultra_compact
        self._canvas.setMinimumHeight(230 if not self._compact else (168 if self._ultra_compact else 190))

        self._summary_strip = QFrame(self)
        self._summary_strip.setObjectName("RRChartSummaryStrip")

        summary_layout = QHBoxLayout(self._summary_strip)
        summary_layout.setContentsMargins(
            9 if not self._compact else (6 if self._ultra_compact else 7),
            6 if not self._compact else (4 if self._ultra_compact else 5),
            9 if not self._compact else (6 if self._ultra_compact else 7),
            6 if not self._compact else (4 if self._ultra_compact else 5),
        )
        summary_layout.setSpacing(7 if not self._compact else (4 if self._ultra_compact else 5))

        self._summary_dot = QLabel(self._summary_strip)
        self._summary_dot.setFixedSize(
            10 if not self._compact else (7 if self._ultra_compact else 8),
            10 if not self._compact else (7 if self._ultra_compact else 8),
        )
    ...
    def _sync_compact_mode(self, force: bool = False) -> None:
        ...
        try:
            self.setMinimumHeight(max(220, int(360 if not self._compact else (260 if self._ultra_compact else 308))))
            ...
            if layout is not None:
                ...
                layout.setSpacing(8 if not self._compact else (4 if self._ultra_compact else 6))

            if hasattr(self, "_canvas") and self._canvas is not None:
                self._canvas._compact = self._compact
                self._canvas._ultra_compact = self._ultra_compact
                self._canvas.setMinimumHeight(230 if not self._compact else (168 if self._ultra_compact else 190))
                if hasattr(self._canvas, '_sync_compact_mode'):
                    self._canvas._sync_compact_mode(force=True)

            if hasattr(self, "_summary_dot") and self._summary_dot is not None:
                dot = 10 if not self._compact else (7 if self._ultra_compact else 8)
                self._summary_dot.setFixedSize(dot, dot)
            ...
            self._logger.exception("Failed to sync compact mode for RRChartWidget")
    ...
    def _apply_shadow(self) -> None:
        ...
        shadow.setBlurRadius(24 if not self._compact else (14 if self._ultra_compact else 18))
        shadow.setOffset(0, 6 if not self._compact else (3 if self._ultra_compact else 4))
    ...
    def _apply_style(self) -> None:
        ...
        radius = 24 if not self._compact else (16 if self._ultra_compact else 18)

        self.setStyleSheet(
            f"""
            QFrame#RRChartWidget {{
        ...
        if self._shadow_effect is not None:
            ...
            self._shadow_effect.setBlurRadius(22 if self._ultra_compact else (28 if self._hovered else 24))
    ...
    def set_ranges(self, ranges: Iterable[RRRangeDefinition]) -> None:
        normalized = [item for item in ranges if isinstance(item, RRRangeDefinition)]
        if not normalized:
            normalized = list(DEFAULT_RR_RANGES)
        ...
        inferred_min = min(mins) if mins else 6.0
        inferred_max = max(maxs) if maxs else 30.0

        inferred_min = min(6.0, inferred_min - 4.0)
        inferred_max = max(30.0, inferred_max + 4.0)
    ...
    def active_range_definition(self) -> Optional[RRRangeDefinition]:
        payload = self._canvas.active_range_payload()
        if not payload:
            return None
        return RRRangeDefinition(
    ...
    def apply_measurement_payload(
        ...
        *,
        ...
        Example:
        measurements = {"respiratory_rate": 18}
        classifications = {
            "respiratory_rate": {
                "label": "Normal",
                "severity": "normal",
        ...
        if isinstance(measurements, Mapping):
            rr_value = measurements.get(
                METRIC_RR,
                measurements.get("respiratory_rate", measurements.get("rr")),
            )

        self.set_value(rr_value, animated=animated, emit_signal=False)

        history = []
        if isinstance(measurements, Mapping):
            raw_history = measurements.get(
                "rr_history",
                measurements.get("recent_rr_values", measurements.get("respiratory_rate_history", [])),
            )
        ...
        if isinstance(classifications, Mapping):
            raw = classifications.get(
                METRIC_RR,
                classifications.get("respiratory_rate", classifications.get("rr", {})),
            )
    ...
    def apply_rr_payload(self, payload: Mapping[str, Any], *, animated: bool = False) -> None:
        """
        Apply a direct respiratory-rate widget payload.

        Supported structure:
        {
            "title": "Respiratory Rate Chart",
            "subtitle": "Resting respiratory-rate interpretation",
            "unit": "breaths/min",
            "value": 18,
            "recent_values": [16, 17, 18],
            "summary": "Respiratory rate is within the common adult resting range.",
            "asset_path": ".../rr_reference_chart.png",
            "ranges": [...]
        }
        """
        ...
        if "unit" in data:
            self.set_unit(safe_str(data.get("unit"), "breaths/min"))
        ...
        if isinstance(raw_ranges, list) and raw_ranges:
            normalized_ranges: List[RRRangeDefinition] = []
            for item in raw_ranges:
                if isinstance(item, RRRangeDefinition):
    ...
    def _update_summary_text(self) -> None:
        ...
        if value is None:
            self._summary_label.setText("Respiratory-rate value is not available yet.")
        elif current is None:
            self._summary_label.setText(
                f"Respiratory rate {self._canvas._format_value(value)} {self._unit} does not match a configured range."
            )
        else:
            ...
            if current.description.strip():
                self._summary_label.setText(
                    f"Respiratory rate {value_text} {self._unit} falls in the “{current.label}” range. "
                    f"{current.description} {trend_text}"
                )
            else:
                self._summary_label.setText(
                    f"Respiratory rate {value_text} {self._unit} falls in the “{current.label}” range. {trend_text}"
                )
    ...
    def _refresh_visibility(self) -> None:
        compact_height = self.height() > 0 and self.height() <= 250
        ultra_tight = bool(self._ultra_compact and (compact_height or self.width() <= 760))

        self._header.setVisible(self._show_header)
        self._title_label.setVisible(bool(self._title.strip()))
        self._subtitle_label.setVisible(bool(self._subtitle.strip()) and not self._ultra_compact)
        self._summary_strip.setVisible(self._show_summary and not ultra_tight)
        self._summary_label.setVisible(self._show_summary and not ultra_tight)
    ...
    def paintEvent(self, event) -> None:
        ...
        try:
            ...
            if rect.width() > 4.0 and rect.height() > 4.0:
                radius = float(24 if not self._compact else (16 if self._ultra_compact else 18))
                path = QPainterPath()
                ...
                gloss_rect = QRectF(
                    ...
                    max(0.0, rect.height() * (0.24 if self._ultra_compact else 0.30)),
                )
    ...
    def diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ultra_compact": getattr(self, "_ultra_compact", False),
